...
def purchaseRetail(request):
    ...
    print(items)
    return render(request, 'invoicepurchRetail.html', context={'items': items, 'head': head, 'form': form})

@require_POST
@csrf_exempt
def process_purchRetail(request):
    print("enter purchase view")
    if request.method == 'POST':
        print('POST request received')
        try:
            ...
            for product in selected_products:  
                ...
                for p in product_list:
                    ...
                    new_invoice = Purchase_model(
                        ...
                        rate = ratee,
                        amt=amount,
                        num=bill_number,  # Ensure all products have the same billnum
                        mode=payment_mode,
                        selbuy=seller_buyer,  # Now passing the actual Customer instance
                        user = request.user
                    )
            ...
            print(f"Error processing order: {e}")
    return render(request, 'invoicepurchRetail.html', context={})
###overviews
##@allowed_users(allowed_roles=['admin'])
